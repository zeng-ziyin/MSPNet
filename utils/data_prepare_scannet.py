...
anno_paths = glob.glob(os.path.join('/media/zengziyin/ZZY/Data/PC/val', '*.ply'))

# gt_class = [x.rstrip() for x in
#             open('/media/cug210/data/ZZY/toronto_unprocess/new_toronto_3d/toronto_3d_classes_8.txt')]
# gt_class2label = {cls: i for i, cls in enumerate(gt_class)}

sub_grid_size = 0.04
original_pc_folder = join('/media/zengziyin/ZZY/Data/ScanNet/val', 'original_ply')
sub_pc_folder = join('/media/zengziyin/ZZY/Data/ScanNet/val', 'input_{:.3f}'.format(sub_grid_size))
...
def convert_pc2ply(anno_path, save_path):
    ...
    process_data = PlyData.read(anno_path)
    process_data = process_data.elements[0].data[:]
