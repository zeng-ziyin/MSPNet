...
from MSPNet import Network
from tester_HRHDHK import ModelTester
from helper_ply import read_ply
from helper_tool import ConfigHRHDHK as cfg
...
class HRHDHK:
    def __init__(self):
        self.name = 'HRHDHK'
        root_path = '/media/zengziyin/ZZY/Data/HRHD-HK'  # path to the dataset
        self.path = root_path
        self.label_to_names = {0: 'Building', 1: 'Vegetation', 2: 'Road', 3: 'Waterbody',
                               4: 'Facility', 5: 'Terrain', 6: 'Vehicle'}
        ...
        self.val_file_name = read_from_txt('/media/zengziyin/ZZY/Data/HRHD-HK/valfile.txt')
        self.test_file_name = read_from_txt('/media/zengziyin/ZZY/Data/HRHD-HK/testfile.txt')
    ...
    def load_sub_sampled_clouds(self, sub_grid_size):
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            c_name = file_path.split('/')[-1]
            print(cloud_name)
            if c_name in self.test_file_name:
                cloud_split = 'test'
            elif c_name in self.val_file_name:
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            if c_name in self.val_file_name:
                proj_file = join(tree_path, '{:s}_proj.pkl'.format(c_name[:-4]))
    ...
    def get_tf_mapping2():
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            ...
            input_up_samples = []
            input_labels = []
            ori_batch_labels = batch_labels

            for i in range(cfg.num_layers):
                neighbour_idx = tf.py_func(DP.knn_search, [batch_xyz, batch_xyz, cfg.k_n], tf.int32)
                sub_points = batch_xyz[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                sub_labels = batch_labels[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i]]
            ...
            input_list += [batch_features, ori_batch_labels, batch_pc_idx, batch_cloud_idx]
